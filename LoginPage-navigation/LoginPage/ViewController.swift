    ...
    @IBAction func forgotPasswordTapped(_ sender: Any)
    {
        ...
        forgotPasswordAlert.addAction(UIAlertAction(title: "Reset Password", style: .default, handler:{ (action) in
            ...
            Auth.auth().sendPasswordReset(withEmail: resetEmail!, completion: { (Error) in
                if Error != nil
                {
                    let resetFailedAlert = UIAlertController(title: "Reset Failed", message: "Error: \(String(describing: Error?.localizedDescription)), Please try again. ", preferredStyle: .alert)
    ...
    func sendVerificationEmail()
    {
        Auth.auth().signIn(withEmail: emailTextField.text!, password: passwordTextField.text!)
        {
            ...
            Auth.auth().currentUser?.sendEmailVerification(completion: { (Error) in
                if Error != nil
                {
                    let emailNotSentAlert = UIAlertController(title: "Email Verification Error", message: "Verification Email Failed to Send: \(String(describing: Error?.localizedDescription))", preferredStyle: .alert)
